...
class Video(object):
	def __init__(self, filename, start=None, end=None, step=None, skip=None,
		...
		infos = self._parse_infos(print_infos=True, check_duration=True)
	...
	def _parse_infos(self, print_infos=False, check_duration=True):
		...
		if "No such file or directory" in lines[-1]:
			raise IOError(
				("Video error: the file %s could not be found !\n"
		...
		if check_duration:
			try:
				...
				raise IOError(
					("Video error: failed to read the duration of file %s.\n"
		...
		if result['video_found']:
			...
			try:
				...
				message = (
					"Video error: failed to read video dimensions in file "
					"%s.\nHere are the file infos returned by ffmpeg:\n\n%s"
				)
	...
	def _read_frame(self):
		...
		if len(s) != nbytes:
			...
			if not hasattr(self, '_lastread'):
				message = (
					"Video error: failed to read the first frame of "
